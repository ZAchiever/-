...
def change_data(full_car_task):
    ...
    for i in close_car:  # 按距离遍历寻找最优可以接单的car
        ...
        empty_car_need_time = 0  # 赶到上货地需要的时间
            ...
            if not set(time_list) & set(car_unable):  # 时间不相交可以选司机了
                ...
                for d in all_driver:
                    ...
                    if not set(time_list) & set(driver_unable_time):  # 抓走合适的司机
                        ...
                        driver_total_time = int(driver.loc[d, 'total_time'])
                        if driver_total_time < 250000:
                            driver_alive.append(d)
                            if len(driver_alive) == 2:  # 司机和汽车准备好了,开始填制表单
                                print('开始载入信息')
                                # 数据备份
                                car.to_excel('car_0.xlsx')
                                driver.to_excel('driver_0.xlsx')
                                history.to_excel('history_0.xlsx')
                                # 重写excel
                                history.loc[len(
                                ...
                                history.loc[len(
                                    all_history), 'driver'] = driver_alive[0]+driver_alive[1]
